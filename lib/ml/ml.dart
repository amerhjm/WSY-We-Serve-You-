...
Future<DataFrame> getDBData() async {
  QuerySnapshot querySnapshot =
      await FirebaseFirestore.instance.collection("training").get();
  List<Object> a = querySnapshot.docs.map((doc) => doc.get("age")).toList();
  List<Object> g = querySnapshot.docs.map((doc) => doc.get("gender")).toList();
  List<Object> c1 = querySnapshot.docs.map((doc) => doc.get("choice")).toList();
  List<String> c = myToInteger(c1);
  var data = List.generate(a.length + 1, (i) => List(3), growable: false);
  data[0] = ['age', 'gender', 'choice'];
  ...
  for (int i = 0, j = 1; i < a.length; i++, j++) {
    ...
    str.write(',');
    str.write(g[i].toString());
    str.write(',');
    str.write(c[i].toString());
    str.write('\r\n');*/
  }
  //str.write("'");
  ...
  /*FirebaseFirestore.instance
    ...
    //List<Object> L = querySnapshot.docs.map((doc) => doc.data()).toList();
    List<Object> a = querySnapshot.docs.map((doc) => doc.get("age")).toList();
    List<Object> g =
        querySnapshot.docs.map((doc) => doc.get("gender")).toList();
    List<Object> c =
        querySnapshot.docs.map((doc) => doc.get("choice")).toList();
    //final allData = querySnapshot.docs.map((doc) => doc.data()).toList();
    StringBuffer str = StringBuffer("'" '\r\nage,gender,choice\r\n');
    //String str = '';
    ...
    str.write("'");
    df = DataFrame.fromRawCsv(str.toString());
  });*/
  ...
  /*FirebaseFirestore.instance
      ...
      .doc("train")
      .get()
      .then((DocumentSnapshot<Map<String, Object>> querySnapshot) {
    if (querySnapshot == null) {
      Fluttertoast.showToast(msg: "1 null", toastLength: Toast.LENGTH_LONG);
    } else {
      if (querySnapshot.exists) {
        if (querySnapshot.data() == null)
          Fluttertoast.showToast(
              msg: "Data don't existe", toastLength: Toast.LENGTH_LONG);
        else
          df = DataFrame.fromJson(querySnapshot.data());
      } else
        Fluttertoast.showToast(
            msg: "Data don't existe", toastLength: Toast.LENGTH_LONG);
    }
...
Future<String> getDBDataS() async {
  QuerySnapshot querySnapshot =
      await FirebaseFirestore.instance.collection("training").limit(200).get();
  List<Object> a = querySnapshot.docs.map((doc) => doc.get("age")).toList();
  List<Object> g = querySnapshot.docs.map((doc) => doc.get("gender")).toList();
  List<Object> c = querySnapshot.docs.map((doc) => doc.get("choice")).toList();
  var data = List.generate(a.length + 1, (i) => List(3), growable: false);
  data[0] = ['age', 'gender', 'choice'];
  //final allData = querySnapshot.docs.map((doc) => doc.data()).toList();
  //StringBuffer str = StringBuffer("'" '\r\nage,gender,choice\r\n');
...
KnnClassifier createClassifier;
List<String> labels = [];
...
// EncoderImpl(DataFrame data, EncoderType label, SeriesEncoderFactoryImpl seriesEncoderFactoryImpl, {Iterable<int> featureIds, Iterable<String> featureNames, String encodedHeaderPostfix, String encodedHeaderPrefix, UnknownValueHandlingType unknownValueHandlingType}) {
// }

/*Future<Vector> evaluateKnnClassifier(MetricType metric, DType dtype) {
  /*final samples = getIrisDataFrame().shuffle().dropSeries(seriesNames: ['Id']);
  final targetName = 'Species';*/
  final samples = getDBData().shuffle().dropSeries(seriesNames: ['age,gender']);
  final targetName = 'choice';
  ...
  createClassifier = (DataFrame trainSamples) => KnnClassifier(
        trainSamples,
        targetName,
        numberOfNeighbours,
        dtype: dtype,
  ...
  /*KnnClassifier c;
  final pred = c.predict(testFeatures);
  pred.
  c.saveAsJson(filePath);*/
...
List<String> myToInteger(List<Object> l) {
  List<String> l_str = l.map((e) => e.toString()).toList();
  labels = l_str.toSet().toList();
  List<String> l1 = List(l.length);
  for (int i = 0; i < l.length; i++)
    l1[i] = (labels.indexOf(l_str[i]) + 1).toString();
  return l1;
}
...
Future<void> generateModel() async {
  /*final samples = getIrisDataFrame().shuffle().dropSeries(seriesNames: ['Id']);
  final targetName = 'Species';
  /*DataFrame d = await getDBData();
  final samples = d.shuffle();
  final targetName = 'choice';*/
  final pipeline = Pipeline(samples, [
    toIntegerLabels(
      columnNames: [targetName],
  ...
  return pipeline.process(samples).header.toString() +
      '     ' +
      pipeline.process(samples).rows.toString();*/
  //DataFrame d = await getDBData();
  /*final samples = getIrisDataFrame().shuffle().dropSeries(seriesNames: ['Id']);
  final targetName = 'Species';*/
  DataFrame d = await getDBData();
  //final samples = d.shuffle().dropSeries(seriesNames: ['age']);
  final samples = d.shuffle();
  final targetName = 'choice';
  final pipeline = Pipeline(samples, [
    /*toIntegerLabels(
  ...
  /*print(createClassifier
      .predict(processed.dropSeries(seriesNames: [targetName]))
      .header);
  ...
  Fluttertoast.showToast(msg: labels.toString());
